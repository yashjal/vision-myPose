...
class CocoFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = '/beegfs/yj627/mypose/train2014/' + self.img_list[index]
	#try:
        img = np.array(cv2.imread(img_path), dtype=np.float32)
	#print img.shape
	#except Exception as e:
	#	print("image exception at index ", index, "and path ", img_path)
	#	return None

        mask_path = '/beegfs/yj627/mypose'+(self.mask_list[index]).strip('.')
        #try:
	mask = np.load(mask_path)
	#except Exception as e:
	#	print("mask exception at index ", index, "and path ", mask_path)
	#	return None
